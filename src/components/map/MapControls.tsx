...
export function MapControls() {
  ...
  const canSeeLiveReps = user?.role === 'regional_manager' || user?.role === 'country_head' || user?.role === 'division_head';
                ...
                {territories.map((territory) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-lg transition-all ${isSelected ? 'bg-white/10' : 'hover:bg-white/5'
                        }`}
                    >
                      <div
                      ...
                      />
                      <span
                        className={`text-xs ${isSelected ? 'text-[#F0F4F8]' : 'text-[#8B9CB8]'
                          }`}
                      >
                        {territory.name}
                      </span>
...
function FilterToggle({ active, onClick, icon, label, color }: FilterToggleProps) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg transition-all ${active ? 'bg-white/10' : 'hover:bg-white/5'
        }`}
    >
      <div
      ...
      <span
        className={`text-xs ${active ? 'text-[#F0F4F8]' : 'text-[#8B9CB8]'
          }`}
      >
        {label}
      </span>
